...
export async function POST(request: NextRequest) {
  ...
  try {
    const evaluated = await db.transaction(async (tx) => {
      ...
      const [fence, previousReport] = await Promise.all([
        getCurrentElectronicFence(actor.id, tx),
        getLatestElectronicFenceLocation(actor.id, tx),
      ])
      if (previousReport?.reportedAt && capturedAt <= previousReport.reportedAt)
      ...
      const result = evaluateFence({
        ...
        isInCustody: ["IN_CUSTODY", "ISOLATION"].includes(
    ...
    if (evaluated.stale)
      return failure("CONFLICT", "定位数据早于最新记录，请重新采集", 409)
    return success(evaluated.data)
  } catch (error) {
