          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Select value={scopeToAdd} onValueChange={setScopeToAdd}>
                  <SelectTrigger className="min-w-0 flex-1"><SelectValue placeholder="选择组织或人员" /></SelectTrigger>
                  <SelectContent>
                    {organizations.data?.map((organization) => { const key = `ORG:${organization.id}`; return <SelectItem key={key} value={key} disabled={scopeKeys.includes(key)}>组织：{organization.name}</SelectItem> })}
                    {users.data?.filter((user) => user.role === "SUPERVISED" && user.status === "active").map((user) => { const key = `USER:${user.id}`; return <SelectItem key={key} value={key} disabled={scopeKeys.includes(key)}>人员：{user.name}</SelectItem> })}
                  </SelectContent>
                </Select>
